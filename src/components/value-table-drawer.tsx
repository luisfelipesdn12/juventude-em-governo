...
export function ValueTableDrawer({ children }: ValueTableDrawerProps) {
    ...
    const handleItemPurchase = async (item: ItemWithCategory) => {
        ...
        try {
            ...
            // Reapply open government card points that give points bonus
            if (currentCityData.open_government_cards && currentCityData.open_government_cards.length > 0) {
                currentCityData.open_government_cards.forEach(openGovCard => {
                    if (openGovCard.reward.type === "points") {
                        if (openGovCard.reward.category_id) {
                            // Apply to specific category
                            newSituationCardsWithAverage.forEach(situationCard => {
                                if (situationCard.categoryId === openGovCard.reward.category_id) {
                                    situationCard.card.metrics.forEach(metric => {
                                        metric.points = Math.min(5, metric.points + openGovCard.reward.quantity);
                                    });
                                    situationCard.points = calculateCardAverage(situationCard.card.metrics);
                                }
                            });
                        } else {
